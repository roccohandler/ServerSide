...
/** Everything in one category, in library order. */
...
/** Everything touching one lifecycle stage, for the diagram's per-stage detail. */
export function byLifecycleStage(
  capabilities: readonly Capability[],
  stage: LifecycleStageId,
): readonly Capability[] {
  return capabilities.filter((capability) => capability.lifecycle.includes(stage));
}
...
/**
 * Groups by tier in the fixed order foundation → recommended → advanced.
 *
 * The order is not the caller's choice, because it is the argument: what you already have,
 * then what you should think about, then what is further off. A caller that could reorder it
 * could accidentally lead with the aspirational group, which is how a capability library
 * turns into a wish list.
 */
export function groupByTier(capabilities: readonly Capability[]): readonly TierGroup[] {
  const order: readonly CapabilityTier[] = ['foundation', 'recommended', 'advanced'];

  return order
    .map((tier) => ({ tier, capabilities: capabilities.filter((c) => c.tier === tier) }))
