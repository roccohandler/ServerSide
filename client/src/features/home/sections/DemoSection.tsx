...
function Demo({ demo }: { readonly demo: DemoExample }) {
  ...
  const panelClass = (side: Side) =>
    [styles['panel'], showing === side ? undefined : styles['panelHidden']]
      .filter(Boolean)
      .join(' ');
    ...
    <Reveal as="li" className={styles['demo']}>
      ...
      <div className={styles['toggle']} role="group" aria-label={`${demo.title}: before or after`}>
        {(['before', 'after'] as const).map((side) => (
          <button
            key={side}
            type="button"
            className={[styles['toggleButton'], showing === side ? styles['toggleOn'] : undefined]
              .filter(Boolean)
              .join(' ')}
      ...
      <div className={styles['panels']}>
        ...
        <div className={panelClass('after')}>
          <p className={`${styles['panelLabel']} ${styles['panelLabelAfter']}`}>
      ...
      <Container>
        ...
        <div className={styles['demoCta']}>
          <p className={styles['demoCtaBody']}>
            These are the same kinds of problems I look for during your free website assessment — on
            your website, not an illustration.
          </p>
          <ButtonLink
            to={primaryCta.to}
            size="lg"
