...
import type { ApiFailure, ApiResult, BillingSummary, CustomerProduct } from '../../../types/api';
import { fetchBilling, openBillingPortal, startCheckout } from '../api/appApi';
...
export function BillingPage() {
  ...
  if (failure) return <AppError failure={failure} onRetry={retry} />;
  if (!billing) return <AppLoading label="Loading your billing" />;
    ...
    <div className={styles['page']}>
      ...
      {justPaid ? (
        <p className={styles['banner']} role="status">
          ...
          <button type="button" className={styles['bannerAction']} onClick={retry}>
            Check again
          </button>
        </p>
      ) : null}
      ...
      {billing.subscription.manageable ? (
        <section className={styles['panel']} aria-labelledby="billing-manage">
          ...
          <Button variant="secondary" disabled={busy} onClick={() => goToStripe(openBillingPortal)}>
