...
import type { CommentThreadView } from '../../../types/api';
...
export function FeedbackThread({ threads, busy, onSubmit }: FeedbackThreadProps) {
  ...
  return (
    <section className={styles['panel']} aria-labelledby="feedback-heading">
      ...
      {threads.length === 0 ? (
        <p className={styles['panelMeta']}>Nothing yet.</p>
      ) : (
        <>
                ...
                {replyTo === thread.id ? (
                  ...
                  >
                    ...
                    <div className={styles['actions']}>
                      <Button type="submit" disabled={busy || replyBody.trim().length === 0}>
                        Send reply
                      </Button>
                      <Button variant="ghost" onClick={() => setReplyTo(null)} disabled={busy}>
                        Cancel
                      </Button>
                    </div>
                  </form>
