...
export const pricing = {
  ...
  /**
   * ==========================================================================
   * CONVERSION FIX — THE SECOND CARD, AND THE DEAD END IT REPLACES
   * ==========================================================================
   *
   * The audit sends a site scoring roughly 65–85% to targeted work rather than a rebuild,
   * because that is the honest answer for a site whose bones are fine. The site's answer to
   * that reader was the phrase "quoted per site" — honest, and a dead end. Somebody who had
   * just scored their own website against twenty checks, the best-qualified visitor this
   * site ever gets, was handed a diagnosis and asked to start a conversation from nothing.
   *
   * So this is a product now: named, scoped, bounded, with a call to action of its own.
   *
   * **The price is not published**, and that is a deliberate state rather than an omission.
   * `conversionFix.pricePublished` is `false` in `config/pricing.ts`; while it stays false
   * the card renders the scope, the boundary and the action, and states that the figure comes
   * out of the free assessment. Setting the flag publishes the figure everywhere at once, and
   * that is the owner's decision — DECISION 014.
   *
   * `excludes` is not fine print. A fixed price is defensible only against a fixed boundary,
   * and the absence of one is exactly why the old $2,500 tier was removed.
   * ==========================================================================
   */
  fix: {
    cadence: 'One-time project',
    name: conversionFix.name,
    statement: conversionFix.statement,
    summary: conversionFix.bestFor,
    price: fixPriceLabel(),
    /** True only when the owner has published the figure. Drives the card's wording. */
    pricePublished: conversionFix.pricePublished,
    priceNote: conversionFix.pricePublished
      ? 'Half to begin, half when the work goes live. Anything the diagnostic finds outside the scope below is quoted before it starts.'
      : 'Scoped from the free assessment and agreed in writing first. Most fixes cost a fraction of a rebuild — and if yours would not, we will say so.',
    includes: conversionFix.includes,
    excludesLabel: 'What it is not',
    excludes: conversionFix.excludes,
    timeline: conversionFix.timeline,
    requestCta: 'Fix my website',
  },
