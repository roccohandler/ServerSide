...
describe('ButtonLink', () => {
  ...
  /*
   * `loading` versus `disabled`, which is an accessibility contract rather than a style.
   *
   * Every busy button on the site used to be `disabled`, and a native disabled button
   * leaves the tab order the moment it becomes disabled. A keyboard user pressing Enter on
   * "Send request" lost focus mid-submit and was returned to the top of the document with
   * nothing announced. These pin the fix: still focusable, announced as unavailable and
   * busy, and not fireable.
   */
  describe('loading', () => {
    it('keeps the button focusable so a keyboard user is not thrown to the top of the page', () => {
    ...
    it('does not fire while loading, which `aria-disabled` alone would allow', async () => {
      const onClick = vi.fn();
      const user = userEvent.setup();
    ...
    it('is independent of `disabled`, because busy and not-applicable are different', () => {
      /* `FeedbackThread`'s reply button is exactly this: busy, or the box is empty. */
      render(
