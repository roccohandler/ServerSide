...
/**
 * The audit's "keep these results" step reads the session — it files the assessment
 * directly for somebody already signed in and sends everybody else to sign up — so the
 * page needs a provider.
 *
 * `initialUser={null}` supplies the answer rather than letting the provider fetch
 * `/api/auth/me`, which in jsdom would be an unhandled request resolving after the
 * assertions. Anonymous is also the state every test in this file is about: this is the
 * public funnel.
 */
...
describe('the website revenue audit', () => {
  ...
  /*
   * The rebuild recommendation names a build; if the reader's trade has a demonstration
   * site, it now also shows one. What is pinned is the honesty of the showing: the
   * capture is the demo's own (its alt describes what is actually visible), the label
   * says "Demonstration", the copy names the business as fictional, and the link goes to
   * the live demo route rather than anywhere aspirational.
   */
  it('shows the trade’s demonstration build beside a rebuild recommendation', async () => {
    const user = userEvent.setup();
    renderAudit();
    ...
    const aside = screen.getByRole('complementary', { name: /hvac demonstration build/i });
    const hvac = portfolioProjects.find((project) => project.id === 'hvac');

    expect(within(aside).getByText('Demonstration')).toBeInTheDocument();
    expect(within(aside).getByText(/fictional business/i)).toBeInTheDocument();
    expect(within(aside).getByRole('img')).toHaveAttribute('alt', hvac?.mobileImageAlt);
    expect(within(aside).getByRole('link', { name: /open the hvac demo/i })).toHaveAttribute(
      'href',
      hvac?.demoUrl,
    );
  });
  ...
  /*
   * The other half of that rule. "Something else" is a first-class trade with no demo by
   * design — see config/demos.ts — so the same rebuild recommendation renders no aside
   * rather than the closest wrong trade's build.
   */
  it('shows no demonstration build for the trade that has none', async () => {
    const user = userEvent.setup();
    renderAudit();
    ...
    // The rebuild branch is on screen; the aside is not.
    expect(screen.getByText(audit.diagnosis.recommendation.heading)).toBeInTheDocument();
    expect(screen.queryByRole('complementary', { name: /demonstration build/i })).toBeNull();
  });
