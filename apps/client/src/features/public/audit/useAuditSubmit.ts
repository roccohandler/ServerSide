...
import { INQUIRY_TYPES, type LeadRequest } from '@jobforge/shared';
import {
  orderErrors,
...
export function useAuditSubmit(audit: AuditSummaryInput): UseAuditSubmitResult {
  ...
  /*
   * The honeypot arrives with the shared submit state, and this form was the only one on
   * the site to have been missing it.
   *
   * Every other lead path renders an off-screen field that a human never sees and a
   * form-filling bot obligingly completes, and `lead.service.ts` silently drops anything
   * that arrives with it set. Without it the audit was the one unguarded door into the
   * owner's inbox, and the one most worth spamming: it is linked from the main navigation
   * and from all five industry pages. Sharing the mechanism is what stops that gap
   * reopening on the next form somebody adds.
   */
  const {
    status,
    honeypotValue,
  ...
  const handleSubmit = useCallback(
    ...
    [audit, begin, fail, honeypotFields, isInFlight, succeed, toIdle, values],
  );
