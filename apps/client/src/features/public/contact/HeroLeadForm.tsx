...
import { Honeypot, RadioGroupField, TextField } from '@jobforge/ui';
  ...
  if (status.kind === 'succeeded') {
    return (
      <div
        ...
        tabIndex={-1}
        role="status"
        className={cx(styles['card'], styles['success'])}
      >
    ...
    <div className={styles['card']}>
      ...
      <div className={styles['progress']} aria-hidden="true">
        {HERO_STEPS.map((name, index) => (
          <span
            key={name}
            className={cx(styles['progressSegment'], index <= stepIndex && styles['progressDone'])}
          />
        ))}
      </div>
