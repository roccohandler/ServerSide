...
import { HONEYPOT_FIELD, type InquiryType } from '@jobforge/shared';
import { Button } from '@jobforge/ui';
import { Icon } from '@jobforge/ui';
import { Honeypot, SelectField, TextAreaField, TextField } from '@jobforge/ui';
  ...
  if (status.kind === 'succeeded') {
    return (
      <div
        ...
        tabIndex={-1}
        role="status"
        className={cx(styles['result'], styles['resultSuccess'])}
      >
    ...
    <>
      {status.kind === 'failed' ? (
        <div
          ...
          tabIndex={-1}
          role="alert"
          className={cx(styles['result'], styles['resultFailure'])}
        >
          <h2 className={styles['resultHeading']}>
