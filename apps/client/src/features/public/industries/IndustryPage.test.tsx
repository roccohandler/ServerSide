...
describe('an industry page', () => {
  ...
  /*
   * Scoped to the demo section deliberately: the hero now renders a second
   * "Demonstration" badge beside its phone capture (pinned separately below), so an
   * unscoped `getByText` would throw on the duplicate. Each rendering of a demo must
   * carry its own visible label — one badge elsewhere on the page does not license an
   * unlabelled screenshot here.
   */
  ...
  /*
   * The hero preview. The audit's finding was that a reader crossed roughly seven
   * hundred words describing what websites look like before this page showed them one.
   * What is pinned: the capture appears in the hero region, it is visibly labelled a
   * demonstration, and its "see it below" link points at an id this same page actually
   * renders — an in-page fragment that matches nothing scrolls nowhere in silence.
   * A trade whose project has no phone capture must render no hero preview at all.
   */
  it('shows the trade demonstration in the hero, labelled and linked to the full card', () => {
    for (const industry of industries) {
      const project = portfolioProjects.find((entry) => entry.id === industry.demoId);
      const { container, unmount } = renderIndustry(industry.slug);

      const hero = screen.getByRole('region', { name: industry.heading });

      if (project?.mobileImage && project.mobileImageAlt) {
        expect(within(hero).getByText('Demonstration')).toBeInTheDocument();
        expect(within(hero).getByRole('img', { name: project.mobileImageAlt })).toBeInTheDocument();

        const link = within(hero).getByRole('link', { name: /see it below/i });
        expect(link).toHaveAttribute('href', '#industry-demo-heading');
        expect(container.querySelector('#industry-demo-heading')).not.toBeNull();
      } else {
        expect(within(hero).queryByText('Demonstration')).toBeNull();
      }
