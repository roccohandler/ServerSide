...
import { DEMO_MAIN_ID, DemoLayout } from './DemoLayout';
...
describe.each(DEMO_SLUGS)('the %s demonstration', (slug) => {
  ...
  it('never renders the marketing site around it', async () => {
    ...
    /*
     * Asserted on `SiteLayout`'s own landmark rather than on link labels, because the
     ...
     *
     * `#main-content` is rendered by `SiteLayout` and by nothing else, so its absence is
     * proof the demo is outside it. That is the property that makes an internal `/demo/...`
     * path defensible at all — see the rewritten `demoUrl` guard in `content.test.ts`, which
     * used to demand an external URL for exactly this reason.
     *
     * This used to assert the *absence of a skip link* as a second proof, and that proxy
     * stopped being true on 2026-08-15 for the opposite of a bad reason: `DemoLayout` grew
     * its own, pointing at its own landmark. A demo has a disclosure bar and a full trade
     * header ahead of its content, so a keyboard user needs a way past them exactly as much
     * here as on the marketing site — arguably more, across fifteen routes. What the link
     * targets is the thing worth asserting, and it is asserted below.
     */
    expect(container.querySelector(`#${MAIN_CONTENT_ID}`)).toBeNull();

    const skipLink = screen.getByRole('link', { name: /skip to main content/i });
    expect(skipLink).toHaveAttribute('href', `#${DEMO_MAIN_ID}`);
