 ...
 *
 * So this is a second shell with the same four responsibilities:
 *
 ...
 *   4. **Survive a throw, and be skippable.** Both were missing until 2026-08-15, and the
 *      sentence at the top of this comment is why: it says `SiteLayout` "paints the JobForge
 *      header, the JobForge footer and the skip link", and only the first two were replaced
 *      here. The skip link is not JobForge chrome — it is the thing that gets a keyboard user
 *      past whatever chrome a page has, and these pages have a disclosure bar and a full
 *      trade header ahead of the content. The error boundary is the same oversight in the
 *      other direction: without one, a demo that throws takes the disclosure bar down with
 *      it, so the page that is left says nothing about being a demonstration.
 * ============================================================================
 */
...
function DemoShell({ site, slug }: DemoOutletContext) {
  ...
  return (
    <div className={styles['shell']} style={paletteStyle(site.palette)}>
      {/*
       * Ahead of the disclosure bar, so it is the first stop in the tab order — a keyboard
       * user reaching the content otherwise passes the bar, the logo, five navigation items
       * and a call element on every one of the fifteen demo routes.
       */}
      <a href={`#${DEMO_MAIN_ID}`} className={styles['skipLink']}>
      ...
      <div className={styles['site']}>
        ...
        <main id={DEMO_MAIN_ID} ref={mainRef} tabIndex={-1} className={styles['main']}>
          {/*
           * Around the routed page only, so a template that throws costs the visitor that
           * page and not the disclosure bar — which is the one element on these routes that
           * must never disappear, because without it the page stops saying it is a
           * demonstration of a business that does not exist.
           */}
          <ErrorBoundary key={pathname} label={`demo ${pathname}`}>
