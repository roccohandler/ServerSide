...
export function DemoPicture({ image, sizes, eager = false, className }: DemoPictureProps) {
  ...
  /*
   * ==========================================================================
   * WHAT A DEMO LOOKS LIKE WHEN ITS PHOTOGRAPHS DO NOT ARRIVE
   * ==========================================================================
   *
   * These pages are the proof asset: a sceptical prospect is sent here to see whether the
   * work is any good. Every one of them is mostly photography, and a browser's broken-image
   * glyph in the middle of that is the single worst thing this site can render — it makes
   * the demonstration of the work look like the work.
   *
   * The failure is not hypothetical either. The images are hashed static assets served from
   * the CDN, and a stale HTML document after a deploy points at hashes that no longer exist.
   * That is a 404 per image, on the pages most likely to be open in somebody's tab.
   *
   * So a failure keeps the *space* — the intrinsic ratio is already declared, so the layout
   * does not jump — and fills it with the demo's own muted surface plus the alt text. The
   * page still reads, still says what the picture was of, and never shows a broken icon.
   *
   * `aria-hidden` on the fallback, because the text in it *is* the alt text: announcing it as
   * an image description and then reading it again as content is the same words twice.
   * ==========================================================================
   */
  if (failed) {
    return (
      <div
        className={cx(styles['mediaFallback'], className)}
        style={{ aspectRatio: `${image.width} / ${image.height}` }}
        role="img"
        aria-label={image.alt}
      >
        <span aria-hidden="true">{image.alt}</span>
      </div>
    );
  }
