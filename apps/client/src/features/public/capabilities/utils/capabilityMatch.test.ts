...
/*
 * The `byCategory`, `byLifecycleStage` and `groupByTier` suites were here and are gone with
 * the functions they covered — see the notes in `capabilityMatch.ts`. All three had no
 * caller in the application, so these tests were the only thing keeping them alive, which
 * is the wrong direction for a test to point.
 */
