...
/*
 * A step had a `label` — "Your email address", "Choose a password" — rendered beside the
 * count on its own line. It is gone, because on every step but one it restated the label
 * of the field directly beneath it, and a heading that repeats its own content is a line
 * of height with no information in it. The fields say what the step is; the count says how
 * much is left. See the note in `CredentialForm.tsx`.
 */
...
const SIGNUP_STEPS: readonly CredentialStep[] = [
  { id: 'email', fields: ['email'], focus: 'email' },
  { id: 'you', fields: ['name', 'businessName'], focus: 'name' },
  { id: 'password', fields: ['password', 'confirmPassword'], focus: 'password' },
];

const LOGIN_STEPS: readonly CredentialStep[] = [
  { id: 'email', fields: ['email'], focus: 'email' },
  { id: 'password', fields: ['password'], focus: 'password' },
];
