...
export function AuthLayout() {
  ...
  useEffect(() => {
    ...
    /*
     * `main`, not `window`. The shell is exactly one viewport tall and `main` is the only
     * thing in it that scrolls — see the note at the top of `AuthLayout.module.css` — so
     * `window.scrollTo` moved a document that no longer has anywhere to go, and a visitor
     * who had scrolled a long card would arrive at the next page part-way down it.
     *
     * The property rather than `scrollTo`: it is the same instant jump with no behaviour
     * to negotiate, and it does not depend on a method jsdom leaves unimplemented.
     */
  ...
  return (
    <div className={styles['shell']}>
      {/*
       * A skip link, which this shell did not have.
       *
       * It looks like the one control ahead of the content makes it unnecessary, and that is
       * exactly backwards on the page it matters most: `/get-my-assessment` is where every
       * primary call to action on the site lands, and a keyboard user arriving there tabbed
       * through the back control and the wordmark link before reaching the field they came to
       * fill in. Two stops is not many; two stops on the funnel's front door, on every visit,
       * is worth one line.
       */}
      <a href={`#${AUTH_CONTENT_ID}`} className={styles['skipLink']}>
