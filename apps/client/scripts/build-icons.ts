 ...
 *
 * ============================================================================
 * WHY THIS ONE SCRIPT WRITES INTO `apps/admin` AS WELL
 * ============================================================================
 *
 * There are two marks now — the customer app's charcoal stamp and the console's ember one,
 * the same geometry with the colour roles swapped. Both need the same ICO fallback for the
 * same reason, and the ICO container below is forty lines of binary format that nobody
 * should own two copies of. A second script in `apps/admin` would be exactly that: a
 * duplicate of a file-format encoder, kept in step by hope.
 *
 * This is not the two-frontends rule being bent. That rule is about *bundles* — neither
 * browser bundle may import the other, because two bundles cannot trust one another. This
 * is a build-time tool writing a committed file into a sibling directory by path. Nothing
 * is imported across the boundary and nothing about it reaches a browser.
 *
 * If a third mark ever appears, `MARKS` grows by one line. If the console ever grows its
 * own icon pipeline for some other reason, take its entry out of this list rather than
 * leaving two things writing one file.
 * ============================================================================
 */

const CLIENT = resolve(import.meta.dirname, '..');
const PUBLIC = join(CLIENT, 'public');
const ADMIN_PUBLIC = resolve(CLIENT, '../admin/public');

/** Every mark that needs an ICO, and the directory holding its source SVG. */
const MARKS: readonly { readonly name: string; readonly directory: string }[] = [
  { name: 'customer app', directory: PUBLIC },
  { name: 'owner console', directory: ADMIN_PUBLIC },
];
...
/**
 * How far into the file sharp will look for the SVG signature.
 *
 * libvips sniffs a buffer's format from its head rather than from its extension, so a file
 * whose `<svg` sits behind a long comment is reported as `Input buffer has corrupt header`
 * — a message about a file that is perfectly valid SVG and renders fine in a browser. This
 * repository writes long rationale comments, which makes that a trap it will walk into
 * again, so the limit is checked here and explained rather than left to be rediscovered.
 *
 * The window is comfortably conservative: libvips looks further than this, and the two
 * marks' comments are both well inside it.
 */
const SIGNATURE_WINDOW_BYTES = 1024;

async function buildIco(directory: string, name: string): Promise<void> {
  const source = await readFile(join(directory, 'favicon.svg'));

  const signatureAt = source.indexOf('<svg');
  if (signatureAt < 0 || signatureAt > SIGNATURE_WINDOW_BYTES) {
    throw new Error(
      `[icons] ${name}: the \`<svg\` element starts ${signatureAt < 0 ? 'nowhere' : `${signatureAt} bytes`} into ` +
        `${join(directory, 'favicon.svg')}, and sharp only looks at the first ${SIGNATURE_WINDOW_BYTES}. ` +
        'Shorten the comment above it — the long-form rationale belongs in docs/design-system.md.',
    );
  }
  ...
  console.log(
    `[icons] ${name}: favicon.ico — ${SIZES.join('/')} px, ${(ico.byteLength / 1000).toFixed(1)} kB`,
  );
}

async function main(): Promise<void> {
  /* Sequential rather than concurrent: two marks, and an ordered log is worth more here
   * than the few milliseconds a `Promise.all` would save. */
  for (const mark of MARKS) {
    await buildIco(mark.directory, mark.name);
  }
}
