...
import type { OnboardingRepository } from '../features/onboarding/onboarding.repository.js';
import type {
  NewOnboardingRecord,
  StoredOnboarding,
...
export function createInMemoryAuthRepository(
  ...
  return {
    ...
    async listUsers(limit: number, search?: string | undefined) {
      /* The same anchored, case-insensitive prefix the repository builds — see `lib/search.ts`. */
      ...
      return [...users]
        .filter(
          (user) =>
            !term ||
            [user.email, user.name, user.businessName ?? ''].some((field) =>
...
export function createInMemoryProjectRepository(
  ...
  return {
    ...
    async listAll(limit: number, search?: string | undefined) {
      /* The same anchored, case-insensitive prefix the repository builds — see `lib/search.ts`. */
      const term = search?.trim().toLowerCase();
      ...
      return [...projects]
        .filter(
          (project) =>
            !term ||
            [project.businessName, project.contactName, project.email].some((field) =>
              field.toLowerCase().startsWith(term),
            ),
        )
        .sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime())
    ...
    async findByEmail(email: string) {
      ...
      return (
        [...projects]
          .filter((project) => project.email.toLowerCase() === wanted)
          .sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime())[0] ?? null
      );
    },

    async listByIds(ids: readonly string[]) {
      /* Unordered, matching the `$in` query — a caller that needs an order must impose one. */
...
export function createInMemoryFeedbackRepository(
  ...
  return {
    ...
    async listAwaitingTeamReply(limit: number) {
      const answered = new Set(
        comments
          .filter((comment) => comment.parentId && comment.authorRole === 'team')
          .map((comment) => comment.parentId),
      );

      /*
       * Bounded before filtering, exactly as the Mongo implementation is — a fake that
       * quietly returns a fuller list than the real one is a fake that hides the bound.
       */
      return comments
        .filter(
          (comment) =>
            !comment.parentId && !comment.resolvedAt && comment.authorRole === 'customer',
        )
        .sort((a, b) => a.createdAt.getTime() - b.createdAt.getTime())
        .slice(0, limit)
...
export function createInMemoryActivityRepository(
  ...
  return {
    ...
    async countForUserSince({ userId, since, customerVisibleOnly, cap }) {
      /* `>` and not `>=`, matching the index query — see the note on the repository method. */
      return Math.min(
        entries.filter(
          (entry) =>
            entry.userId === userId &&
...
/**
 * The welcome-form submissions.
 *
 * Here rather than inside one test file because two things need it now: the service's own
 * tests, and the platform harness — the console's project page reads the matched submission,
 * so an app built without this reaches for a database and answers 503 to a route that has
 * nothing to do with onboarding.
 */
export function createInMemoryOnboardingRepository(
  options: { now?: () => Date } = {},
): OnboardingRepository & { readonly records: StoredOnboarding[] } {
  const now = options.now ?? (() => new Date());
  const records: StoredOnboarding[] = [];
  let nextId = 1;

  return {
    records,

    async create(record: NewOnboardingRecord) {
      const timestamp = now();
      const stored: StoredOnboarding = {
        ...record,
        id: `onboarding-${nextId++}`,
        createdAt: timestamp,
        updatedAt: timestamp,
      };
      records.push(stored);
    ...
    async listUnmatched(limit: number) {
      /* Oldest first, matching the repository: the longest wait is the most expensive one. */
      return records
    ...
    async attachProject(id: string, projectId: string) {
      const index = records.findIndex((record) => record.id === id);
      if (index === -1) return null;

      const updated: StoredOnboarding = {
        ...(records[index] as StoredOnboarding),
        projectId,
        updatedAt: now(),
      };
