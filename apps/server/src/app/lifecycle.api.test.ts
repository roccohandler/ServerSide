...
describe('the customer lifecycle', () => {
  ...
  it('runs from assessment to live website', async () => {
    ...
    /* --------------------------------------- 12. the second instalment */

    /*
     * ======================================================================
     * THE BALANCE OUTRANKS THE SUBSCRIPTION, AND THIS STEP IS WHY IT EXISTS
     * ======================================================================
     *
     * This step is new, and it was added because this test failed: the walk went straight
     * from `live` to `choose-plan`, and that assertion had been correct for as long as the
     * portal could not take the second payment. It can now, so the dashboard's answer at
     * this exact point changed — a customer whose site is live and whose balance is
     * outstanding is offered the balance, not a subscription.
     *
     * That ordering is the thing worth pinning. Offering somebody a monthly service for
     * looking after a website they have not finished paying for reads as a business that is
     * not paying attention, and it is the kind of defect that is invisible in a unit test of
     * either branch on its own.
     * ======================================================================
     */
    const owing = await authed('get', '/api/app/dashboard');
    expect(owing.body.data.currentAction.kind).toBe('pay-final');
    expect(owing.body.data.billing.available.final).toBe(true);
    expect(owing.body.data.billing.available.plan).toBe(true);

    const finalCheckout = await authed('post', '/api/app/billing/checkout').send({
      product: 'build-final',
    });
    expect(finalCheckout.status).toBe(201);

    /* Again: reaching Stripe changed nothing. Only the signed event does. */
    expect(harness.repositories.projects.projects[0]?.finalStatus).toBe('pending');

    /*
     * `userId`, not `projectId`, and that is the point of the step.
     *
     * `createCustomerCheckoutSession` builds the metadata from the session — a browser must
     * never be able to name the project a payment settles — so the webhook comes back naming
     * an *account*. Resolving that to a project is `settleFinalPayment`'s whole job, and a
     * test that sent a `projectId` here would exercise the owner-link path instead and pass
     * against a build where the self-serve one does nothing at all.
     */
    const settled = await deliverStripeEvent({
      id: 'evt_final_paid',
      type: 'checkout.session.completed',
      object: {
        payment_status: 'paid',
        customer: 'cus_test_1',
        payment_intent: 'pi_final_1',
        metadata: { userId, product: 'build-final' },
      },
    });

    expect(settled.status).toBe(200);
    expect(harness.repositories.projects.projects[0]?.finalStatus).toBe('paid');

    /* ------------------------------------------- 13. the recurring service */
    ...
    /* Settled, so it is no longer on offer — the same rule the deposit follows. */
    expect(finalDashboard.body.data.billing.available.final).toBe(false);
  ...
  describe('payment edge cases', () => {
     ...
     * ======================================================================
     *
     * The billing page renders a control per `available.*` flag, and that is presentation:
     * a request typed by hand never went near it. These three assert the *server* refuses,
     * which is the only place a refusal counts.
     *
     * The launch instalment is the one that matters most, because it is the only product
     * whose availability turns on the state of a build rather than on a payment status —
     * so it is the one where an out-of-order request is a plausible mistake rather than an
     * obvious attack. Selling the second half of a build that has not started is what the
     * gate exists to make impossible.
     * ======================================================================
     */
    it('refuses a launch payment before there is a build to settle', async () => {
      await deliverStripeEvent({
        id: 'evt_deposit_for_gate',
        type: 'checkout.session.completed',
        object: {
          payment_status: 'paid',
          customer: 'cus_test_1',
      ...
      /* The project exists and is in onboarding: deposit paid, nothing built. */
      const response = await authed('post', '/api/app/billing/checkout').send({
        product: 'build-final',
      });

      expect(response.status).toBe(400);
      expect(response.body.error.message).toMatch(/going live/i);
      expect(harness.stripe.checkoutRequests).toHaveLength(0);
    });

    it('refuses a second deposit once the first has cleared', async () => {
      await deliverStripeEvent({
        id: 'evt_deposit_for_repeat',
        type: 'checkout.session.completed',
        object: {
          payment_status: 'paid',
          customer: 'cus_test_1',
    ...
    /*
     * The published terms say Growth Partner starts on launch day. The dashboard's
     * `choose-plan` action was already gated on `milestone === 'live'`; this is the endpoint
     * agreeing with it, which it did not before.
     */
    it('refuses the plan before there is a website to look after', async () => {
      const response = await authed('post', '/api/app/billing/checkout').send({
        product: 'growth-partner-monthly',
      });

      expect(response.status).toBe(400);
      expect(response.body.error.message).toMatch(/launch day/i);
      expect(harness.stripe.checkoutRequests).toHaveLength(0);
    });
