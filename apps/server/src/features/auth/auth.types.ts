...
export interface StoredUser {
  ...
  /**
   * When this person last caught up on their own activity stream.
   *
   * ## Why a single marker and not a read flag per event
   *
   * Because "unread" is a property of the *reader*, not of the thing read. The alternative —
   * a `Notification` collection with a `readAt` on every row — would be a second record of
   * what happened, sitting beside `ActivityModel`, which already records every one of these
   * events for the timeline the customer reads. Two tables answering "what happened" is how
   * they come to disagree, and the one nobody is looking at is the one that drifts.
   *
   * One timestamp gives the only thing the interface actually asks: *is there anything here
   * I have not seen*. It cannot express "read the third item and not the first", and nothing
   * in this product wants to.
   *
   * Absent until they first open the dashboard, which is deliberately different from zero:
   * a brand-new account has nothing to be behind on, so the count is computed against
...
export interface AdminAccountView {
  ...
  /**
   * Whether this account has ever sent a request — from the contact form or from the
   * request step of the account-first funnel.
   *
   * The one field on this view that is not a property of the user document, and it is here
   * because it is the question the table exists to answer since DECISION 028: an account
   * with `false` is somebody who clicked "Get my free website assessment", chose a password,
   * and then stopped. Before the funnel changed, that person left no trace at all.
   *
   * Optional rather than required, because it is composed in by the admin route from a
   * second query and `toAdminAccountView` is given a `StoredUser` and nothing else. A mapper
   * that took a lead index as a parameter would be a mapper that could not be called without
   * one, on the two other surfaces that do not care.
   */
  readonly hasRequested?: boolean | undefined;
}
...
export interface UserUpdate {
  ...
  /**
   * Safe on this object in a way `role` is not, and the difference is worth stating because
   * the role-granting write is kept off it deliberately (see the repository): the worst a
   * mass-assignment mistake can do with this field is tell somebody they are up to date on
   * their own activity. It grants nothing and reveals nothing.
   */
  readonly activityReadAt?: Date;
}
