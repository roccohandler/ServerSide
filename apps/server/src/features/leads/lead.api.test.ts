...
/**
 * A repository whose submission path is stubbed and whose read path is inert.
 *
 * `findById`, `listAwaitingReply` and `updateStatus` exist for the owner console and are
 * not on the submission path at all. They answer emptily rather than being given
 * behaviour nothing here exercises — a stub that pretends to have data is a stub that
 * makes the next reader wonder which test depends on it.
 */
function stubLeadRepository(overrides: Partial<LeadRepository>): LeadRepository {
  return {
    create: () => Promise.reject(new Error('`create` was not stubbed for this test.')),
    findRecentDuplicate: () => Promise.resolve(null),
    updateNotificationStatus: () => Promise.resolve(),
    findById: () => Promise.resolve(null),
    listAwaitingReply: () => Promise.resolve([]),
    updateStatus: () => Promise.resolve(),
    findUserIdsWithLeads: () => Promise.resolve(new Set<string>()),
    hasLeadForUser: () => Promise.resolve(false),
    ...overrides,
  };
}
...
describe('POST /api/leads', () => {
  ...
  it('tells the visitor to call when persistence is unavailable', async () => {
    const unavailable = stubLeadRepository({
  ...
  it('never leaks internal failure detail in production', async () => {
    const exploding = stubLeadRepository({
  ...
  it('includes debug detail for unexpected errors in development only', async () => {
    const exploding = stubLeadRepository({ create: () => Promise.reject(new Error('boom')) });
